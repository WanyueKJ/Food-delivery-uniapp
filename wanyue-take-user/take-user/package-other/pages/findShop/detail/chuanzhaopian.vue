<template>
	<view>
		<wybar @clickLeft="back" :leftText="i18n.toher.chuanzhaopian"></wybar>
		<view class="column" style="margin-top: 10px;width: 100%;background-color: #FFF;">
			...
			<view class="row" style="flex-wrap: wrap;margin-left: 30rpx;width: 690rpx">
				<view v-for="(item,index) in imgList" :key="index"
					style="width: 172.5rpx;height: 172.5rpx;align-items: center;justify-content: center;display: flex;position: relative;margin-top: 10rpx;margin-bottom: 10rpx;">
					<image :src="item" mode="aspectFill" style="width: 155rpx;height: 155rpx;border-radius: 8rpx;">
					</image>
				...
				<view class="column"
					style="align-items: center;justify-content: center;width: 172.5rpx;height: 172.5rpx;margin-top: 10rpx;margin-bottom: 10rpx;"
...
<script>
	...
	export default {
		...
		methods: {
			...
			chuanzhaopian(list) {
				var that = this
				that.http.sendRequest('LookingShop.SendPhoto', {
					store_id: that.storeID,
					banner: JSON.stringify(list)
				}).then(function(res) {
					if (res.code == 0) {
						uni.showToast({
							title:res.msg,
