<template>
	<view>
		<view v-if="refundMsg" class="showView column">
			<view style="width: 100%;height: 9px;background-color: #FD9264;">
			...
			<text style="width: calc(100% - 24px);margin-top: 5px;color: #646464;font-size: 14px;margin-left: 12px;padding-bottom: 15px;border-bottom: 1px solid #fafafa;" v-if="refundMsg.status == 9">平台审核未通过，退款失败</text>
			<view v-for="(item,index) in array" :key="index" class="row" style="width: calc(100% - 24px);height: 40px;margin-left: 12px;" >
				...
				<text style="color: #F30700;font-size: 18px;height: 40px;line-height: 40px;font-weight: bold;margin-right: 1px;" v-if="index == 0">{{'¥' + refundMsg.money}}</text>
			...
			<view v-for="(item,index) in listArray" :key="index" class="row" style="width: calc(100% - 24px);margin-left: 12px;">
				<view class="column">
					<view class="circle_quan" v-if="index == 0">
					</view>
					<view class="circle_shi" v-else>
					</view>
					<view class="xian" v-if="index != (listArray.length - 1)">
...
<script>
	export default {
		...
		onLoad(option) {
			...
			that.http.sendRequest('Orders.GetRefund',{'orderid':that.orderID}).then(function(res){
				if(res.code == 0){
