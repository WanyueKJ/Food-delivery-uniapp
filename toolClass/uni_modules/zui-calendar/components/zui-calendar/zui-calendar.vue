		...
		<view class="date-box">
			<view class="box-list">
				...
				<view class="date-week">
					<view class="week-item" v-for="item in weekList"><text>{{item}}</text></view>
				</view>
				<view class="day-content">
					<view class="day-item day-month"><text>{{month < 10 ? `0${month}` : month}}</text></view>
					<!--非 h5 平台 :key 不支持表达式 z-${item.day}-${index} -->
					<view 
						class="day-item" 
	...
	export default {
		...
		methods: {
			...
			toActive(item, index){
				...
				var newdate = this.year + '-'+newmonth+'-' + newday
				this.$emit('click-active', {year:this.year, month:this.month, day:item.day, date:newdate, index: index})
			},
