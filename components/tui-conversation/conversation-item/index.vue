<template>
	<!--TODO: 默认图片在 cos 上添加 :data-index="index" list-touch -->
	<movable-area v-if="conversation.conversationID" class="t-conversation-item-container">
		<movable-view class="t-conversation-item list-touch" direction="horizontal" @change="handleTouchMove"
			...
			<view class="t-conversation-delete" @tap.stop="deleteConversation">{{i18n.mine.shanchu}}</view>
		</movable-view>
	</movable-area>
...
<script>
	...
	export default {
		...
		methods: {
			...
			deleteConversation() {
				uni.showModal({
					content: this.i18n.message.suredelete,
					success: res => {
						if (res.confirm) {
