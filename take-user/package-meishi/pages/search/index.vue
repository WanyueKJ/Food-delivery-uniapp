<template>
	<view>
		...
		<view class="row topSearch">
			...
			<input :value="keywords" class="sousuokuang" :placeholder="i18n.index.search" type="text" confirm-type="search"
				@confirm="doSearch">
		</view>
		<blockquote v-if="listArray.length > 0">
			...
			<scroll-view scroll-y="true" style="width: 100%;background-color: #f5f6f7;"
				...
				<view v-for="(obj,j) in listArray" :key="j" class="column cellBack">
					<view class="cell row" @click="gotoGoods(obj)">
						...
						<view class="column" style="width: calc(100% - 134px);">
							...
							<view class="row store_label">
								...
								<!-- <image src="../../../static/meishi/zan-nor.png" style="margin-left: 10px;" mode=""> -->
								<!-- </image> -->
					...
					<view style="margin-left: 12px;width: calc(100% - 24px);height: 1px;background-color: #f5f6f7;">
					</view>
					<view class="row" style="width: 100%;height: 62px;align-items: center;"
						@click="gotoStore(obj.store)">
						<image :src="obj.store.thumb" mode="" class="cell_thumb" style="width: 46px;height: 35px;">
						</image>
						<view class="column" style="width: calc(100% - 70px);">
							...
							<view class="row" style="align-items: center;">
								<text
									style="font-size: 14px;font-weight: bold;color: #FF5725;margin-left: 3px;">{{obj.store.stars}}</text>
								<text
		...
		methods: {
			...
			gotoGoods(obj) {
				if (this.ishome == 1) {
					...
					} else if (obj.store.top_type_id == 4) {
